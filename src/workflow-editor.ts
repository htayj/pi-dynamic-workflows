...
export function buildForcedWorkflowPrompt(text: string, extraDirective?: string): string {
  const lines = [
    ...
    "Workflows that make intentional file changes in a git-tracked project SHOULD normally include a final `Commit & Push Gate` phase/stage after `Antagonistic Code Review` and `Use Verification` have passed; declare it in meta.phases as `{ title: 'Commit & Push Gate' }`, call phase('Commit & Push Gate'), and run an agent() that checks git status/diff, determines the repo is owned or are authorized to push, runs a leak/secret check over all changes (for example, `gitleaks protect --staged` or equivalent before commit/push), then performs a normal commit and push only if all gates pass. It MUST NOT commit or push when the user said not to, there are no intentional changes, ownership/push authorization is unknown or absent, review/verification failed or has unresolved findings, the action would be destructive/unwanted, or the leak/secret check fails; instead leave changes uncommitted/unpushed and explicitly report why.",
    "",
