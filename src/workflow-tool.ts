...
export function createWorkflowTool(options: WorkflowToolOptions = {}): ToolDefinition<typeof workflowToolSchema, any> {
  ...
  return defineTool({
    ...
    promptGuidelines: [
      ...
      "For workflow, every non-trivial workflow that implements, changes, or drafts code or code-like artifacts must include a distinct `Antagonistic Code Review` phase/stage; declare it in meta.phases as `{ title: 'Antagonistic Code Review' }`, call phase('Antagonistic Code Review'), and run an independent skeptical/hostile reviewer agent after the implementation/artifact draft and before `Use Verification` or final synthesis. The reviewer must inspect changed code, diffs, design, or produced artifacts for bugs, security issues, regressions, missing tests, and whether the workflow satisfied the user request. The workflow must fix or explicitly rebut findings before completion. For no-code workflows, run an analogous antagonistic review of the artifact/answer, or include an explicit note explaining why code review is not applicable.",
      "For workflow, every non-trivial workflow that implements, changes, or verifies application behavior must include a distinct `Use Verification` phase/stage; declare it in meta.phases as `{ title: 'Use Verification' }`, call phase('Use Verification'), and run an agent() in that phase that actually uses the application/system non-destructively to verify the requested feature/change. Code review, tests, or verify() alone do not satisfy use verification. Modality expectations: TUI apps are tested by running/using the app in tmux; web pages by Playwright plus captured screenshots and screenshot/image analysis; GUI apps by computer-use under xvfb plus captured screenshots/images and an image gate; other apps by equivalent real use. Only exception: if use verification would be destructive and there is no non-destructive or sandbox option, include an explicit skipped-verification agent explaining why it was skipped.",
      "For workflow, workflows that make intentional file changes in a git-tracked project should normally include a final `Commit & Push Gate` phase/stage after `Antagonistic Code Review` and `Use Verification` have passed; declare it in meta.phases as `{ title: 'Commit & Push Gate' }`, call phase('Commit & Push Gate'), and run an agent() that checks git status/diff, determines the repo is owned or are authorized to push, runs a leak/secret check over all changes (for example, `gitleaks protect --staged` or equivalent before commit/push), then performs a normal commit and push only if all gates pass. It must not commit or push when the user said not to, there are no intentional changes, ownership/push authorization is unknown or absent, review/verification failed or has unresolved findings, the action would be destructive/unwanted, or the leak/secret check fails; instead leave changes uncommitted/unpushed and explicitly report why.",
